...
#define TILE_WIDTH 16 // Size of the tile
#define FILTER_SIZE 5  // Size of the filter
#define FILTER_RADIUS 2  // The filter radius
// Constructor
...
__global__ void fp_preact_c1( const float input[28][28], float preact[6][24][24],const float weight[6][5][5]) {
...
__global__ void fp_bias_c1(float *preact, const float *bias, int width, int height) {
    int i = blockIdx.z;
...
__global__ void bp_output_c1_kernel(float d_output[6][24][24],const float n_weight[1][4][4],const float nd_preact[6][6][6]) {
...
__global__ void bp_bias_c1_kernel(float bias[6], const float d_preact[6][24][24]) {

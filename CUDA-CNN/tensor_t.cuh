...
template<typename T>
void copyTensorToDevice(const tensor_t<T>& hostTensor, T** deviceDataPtr)
{
    const size_t numBytes = hostTensor.size.x * hostTensor.size.y * hostTensor.size.z * sizeof(T);
    cudaMalloc(deviceDataPtr, numBytes);
    cudaMemcpy(*deviceDataPtr, hostTensor.data, numBytes, cudaMemcpyHostToDevice);
}

template<typename T>
void copyTensorToHost(tensor_t<T>& hostTensor, const T* deviceData)
{
    const size_t numBytes = hostTensor.size.x * hostTensor.size.y * hostTensor.size.z * sizeof(T);
    cudaMemcpy(hostTensor.data, deviceData, numBytes, cudaMemcpyDeviceToHost);
}